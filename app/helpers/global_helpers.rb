module Merb
  module GlobalHelpers
    ...
    def render_players(demo)
      case demo.gametype
      when 'duel'
        players = demo.player_stats.map { |p| render_nickname(p.colored_name) }.join(' vs ')
      else
        demo.player_stats.map { |p| render_nickname(p.colored_name) }.join(', ')
      end
    end
    ...
    def render_nickname(name)
      ...
      html = h(name).gsub(/\^(\d)/){"</span><span class=\"c#{self.color_index($1)}\">"}
